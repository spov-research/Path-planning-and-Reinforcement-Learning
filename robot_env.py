...
class CarDCENV(object):
    ...
    def __init__(self, map_bin, goald, start_point):
        d = pow(2, 0.5)/2
        self.kernel_s = np.array([[1, 0], [d, d], [
                                 0, 1], [-d, d], [-1, 0], [-d, -d], [0, -1], [d, -d]], dtype=np.float64)
        ...
        self.exit = False
        self.fail = False
        self.dist_i = np.array(
            [self.goald[0]-self.start_point[0], self.goald[1]-self.start_point[1]])
    ...
    def step(self, action):
        ...
        self.car_info[:2] = self.car_info[:2] + self.speed * self.dt * \
            np.array([np.cos(self.car_info[2]), np.sin(self.car_info[2])])
        self._update_sensor()
        s = self._get_state()
        ...
        if self.exit:
            ...
            r = 0.4 + (0.2 if s[0] < 1 else 0.001) + \
                (0.4 if dd_n/self.last_dist < 1 else 0.001)
            self.last_r = r
            self.last_s = s
    ...
    def reset(self):
        ...
        self.temp_map = self.mapa.copy()
        cv2.circle(self.temp_map, (int(round(self.goald[0])), int(
            round(self.goald[1]))), 30, (255, 0, 0), 1)
        return self._get_state()

    def render(self):
        cx2, cy2, _ = self.car_info.copy()
        if not self.terminal:
            cv2.circle(self.temp_map, (int(round(cx2)),
                       int(round(cy2))), 5, (255, 0, 0), -1)
    ...
    def _update_sensor(self):
        ...
        if not (np.isnan(cx) or np.isnan(cy)):
            ...
            kernel_s_rot[:, 0] = kernel_s_temp[:, 0] * \
                np.cos(rotation) - kernel_s_temp[:, 1]*np.sin(rotation)
            kernel_s_rot[:, 1] = kernel_s_temp[:, 0] * \
                np.sin(rotation) + kernel_s_temp[:, 1]*np.cos(rotation)
            ...
            for i in range(1, self.sensor_max+1):
                ...
                for sensor_n in range(len(kernel_s_cur)):
                    ...
                    if 0 < round(kernel_s_cur[sensor_n][1]) < self.window[1] and 0 < round(kernel_s_cur[sensor_n][0]) < self.window[0]:
                        # se invierte el orden de X,Y a Y,X, debido a que map es una matriz
                        if self.mapa[int(round(kernel_s_cur[sensor_n][1])), int(round(kernel_s_cur[sensor_n][0]))] == 255:
                            dist = np.array(
                                [kernel_s_cur[sensor_n][0]-cx, kernel_s_cur[sensor_n][1]-cy])
                    ...
                    # Distancia a los límites de la ventana
                        ...
                        px = np.clip(
                            kernel_s_cur[sensor_n][0], 0, self.window[0])
                        py = np.clip(
                            kernel_s_cur[sensor_n][1], 0, self.window[1])
                    ...
                    dist_gl = np.array(
                        [kernel_s_cur[sensor_n][0]-self.goald[0], kernel_s_cur[sensor_n][1]-self.goald[1]])
